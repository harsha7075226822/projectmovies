...
function TrendingMovies() {
  ...
  const TrendingMoviesData = async () => {
      ...
      if (response.ok) {
        ...
        // console.log(data)
        const formattedData = data.data.map(eachMovie => ({
          id: eachMovie.id,
          title: eachMovie.title,
          posterPath: eachMovie.posterPath,
          backdropPath: eachMovie.backdropPath,
    ...
    return (
      <div className="bg-black text-white">
        <div className="relative flex items-center px-2 sm:px-4">
          ...
          <div
            ref={scrollRef}
            className="flex gap-3 sm:gap-4 overflow-x-auto no-scrollbar scroll-smooth py-2 snap-x snap-mandatory"
          >
            {data.map(movie => (
          ...
          <button
            onClick={scrollRight}
            className="absolute right-1 sm:right-0 z-10 bg-black/60 hover:bg-black p-2 rounded-full"
          >
